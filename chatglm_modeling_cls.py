...
class ChatGLMforPolicyModel(ChatGLMPreTrainedModel):
    def __init__(self, config: ChatGLMConfig, empty_init=True, device=None):
        super().__init__(config)
        ...
        # Add a linear layer and initialize weights
        self.policy_head = nn.Linear(config.hidden_size, self.action_space_size, bias=True)
      
        w=torch.normal(0, 0.01, size=(self.action_space_size, config.hidden_size), requires_grad=True)    
        self.policy_head.weight = Parameter(w)
        self.config = config
        self.quantized = False
    ...
    def forward(
        ...
        hidden_states = hidden_states.to(self.policy_head.weight.dtype) 
        logits = self.policy_head(hidden_states)
        logits = logits.float()
        logits = F.softmax(logits, dim=2)
        # Obtain the last logit from the output
        logits = logits[-1]
        return PolicyOutput(
            logits = logits
        )
